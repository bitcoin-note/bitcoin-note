...
#ifdef _WIN32
const std::string DAEMON_FILENAME = "bitnoted.exe";
#else
const std::string DAEMON_FILENAME = "bitnoted";
#endif
...
void BaseFunctionalTests::startNode(size_t index) {
  ...
  config
    ...
    << "log-file=test_bitnoted_" << index << ".log" << std::endl;
...
#if defined WIN32
  std::string commandLine = "start /MIN \"bitnoted" + std::to_string(index) + "\" \"" + daemonPath.string() +
  ...
  if (pid == 0) {
    ...
    if (execl(pathToDaemon.c_str(), "bitnoted", "--testnet", dataDir.c_str(), "--config-file=daemon.conf", NULL) == -1) {
