...
#ifdef _WIN32
const std::string bitnoteDaemon = "bitnoted.exe";
#else
const std::string bitnoteDaemon = "bitnoted";
#endif
...
TestNodeConfiguration TestNetworkBuilder::buildNodeConfiguration(size_t index) {
  ...
  cfg.daemonPath = bitnoteDaemon; // default
  cfg.testnet = testnet;
  cfg.logFile = "test_bitnoted" + std::to_string(index) + ".log";
