...
bool Currency::generateGenesisBlock() {
  ...
  //constructMinerTx(0, 0, 0, 0, 0, 0, ac, tx); // zero fee in genesis
...
CurrencyBuilder::CurrencyBuilder(Logging::ILogger& log) : m_currency(log) {
  ...
  numberOfDecimalUnits(parameters::CRYPTONOTE_DISPLAY_DECIMAL_UNITS);
...
CurrencyBuilder& CurrencyBuilder::numberOfDecimalUnits(uint64_t val) {
  if (val <= 0) {
    throw std::invalid_argument("val at numberOfDecimalUnits()");
  }
